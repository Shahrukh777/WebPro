...
const UserDetails = ({ id, type, subAccounts, userData }: Props) => {
  ...
  const validRoles = ['AGENCY_OWNER', 'AGENCY_ADMIN', 'SUBACCOUNT_USER', 'SUBACCOUNT_GUEST'] as const;
  ...
  const userDataSchema = yup.object({
    ...
    role: yup
    .mixed<typeof validRoles[number]>()
    .oneOf(validRoles) // Restrict to valid roles
    .required(),
    
  })

  const form = useForm<yup.InferType<typeof userDataSchema>>({
  ...
  const onSubmit = async (values: yup.InferType<typeof userDataSchema>) => {
    if (!id) return
